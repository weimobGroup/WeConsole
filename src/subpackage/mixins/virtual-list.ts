...
export class VirtualListMixin<T extends RequireId = RequireId> extends MpComponentMixin {
    ...
    $vlListChange() {
        this.$updateData({
            $vlTotalCount: this.$vlAllList?.length || 0
        });
        this.$vlComputeShowList();
    }
    ...
    $vlComputeShowList() {
        ...
        this.$vlComputeShowListTimer = setTimeout(() => {
            ...
            const oldStart = this.$vlStartIndex || 0;
            const oldEnd = this.$vlEndIndex || 0;
            ...
            const staticHeight =
                this.data.$vlItemStaticHeight && typeof this.data.$vlItemStaticHeight === 'number'
                    ...
                    : null;
            for (let index = 0, len = this.$vlAllList?.length || 0; index < len; index++) {
                ...
                if (top >= (this.$vlScrollTop || 0)) {
                    ...
                    if (visibleHeight >= (this.$vlContainerHeight || 0) && lastIntersectIndex === -1) {
            ...
            needUpdate =
                ...
                (this.$vlPrevScrollInfo[0] !== this.$vlOldScrollTop && this.$vlPrevScrollInfo[1] !== this.$vlScrollTop);
            if (!needUpdate) {
                return;
            }
            this.$vlPrevScrollInfo = [this.$vlOldScrollTop || 0, this.$vlScrollTop || 0];
    ...
    $vlSetShowList(startIndex, endIndex) {
        ...
        if (needUpdate) {
            ...
            this.$vlAllList.forEach((item, index) => {
                ...
                if (index < (this.$vlStartIndex || 0)) {
                    startHeight += itemHeight;
                } else if (index >= (this.$vlStartIndex || 0) && index < (this.$vlEndIndex || 0)) {
                    list.push(item);
                } else {
