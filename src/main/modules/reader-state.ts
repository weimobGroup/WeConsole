...
import { checkMultiplePageStateEnabled } from '../config';
...
export class ReaderStateController extends EventEmitter {
    ...
    setState(key: string, val: any) {
        if (!checkMultiplePageStateEnabled() && key !== 'showIcon') {
            return;
        }
    ...
    removeState(key: string) {
        if (!checkMultiplePageStateEnabled()) {
            return;
        }
    ...
    private handMap(type: string, ...args): string[] | undefined {
        ...
        let map: any;
        let cancelAll: string | undefined;
    ...
    keepSave(id?: string, save?: boolean): string[] {
        return this.handMap.apply(this, ['keepSave', id, save]) || [];
    }

    mark(id?: string, save?: boolean): string[] {
        return this.handMap.apply(this, ['mark', id, save]) || [];
    }

    top(id?: string, save?: boolean): string[] {
